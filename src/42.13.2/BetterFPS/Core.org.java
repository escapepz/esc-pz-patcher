...
public final class Core {
    ...
    public void TakeFullScreenshot(String filename) {
        RenderThread.invokeOnRenderContext(filename, l_filename -> {
            ...
            File file = new File(ZomboidFileSystem.instance.getScreenshotDir() + File.separator + (String)l_filename);
            for (int i = 0; i < pixels.length; ++i) {
                int bindex = i * 3;
                pixels[i] = 0xFF000000 | (buffer.get(bindex) & 0xFF) << 16 | (buffer.get(bindex + 1) & 0xFF) << 8 | (buffer.get(bindex + 2) & 0xFF) << 0;
            }
...
lbl12:
    ...
    public KahluaTable getScreenModes() {
        ...
        try {
            if (!newFile.exists()) {
                ...
                for (int n = 0; n < modes.length; ++n) {
                    w = modes[n].getWidth();
                    if (test.contains(w + " x " + (h = Integer.valueOf(modes[n].getHeight())))) continue;
                    t.rawset(c, (Object)(w + " x " + h));
                    fw.write(w + " x " + h + " \r\n");
                    test.add((CallSite)((Object)(w + " x " + h)));
                    ++c;
                }
                fw.close();
            } else {
    ...
    public String getVersion() {
        return String.valueOf(gameVersion) + ".2 " + Core.getGitRevisionString();
    }
    ...
    public String getGitRevision() {
        if (this.gitRevisionString == null) {
            this.gitRevisionString = IS_DEV ? DevMainScreenState.getDevVersion() : "986d733f73d95810492027afa9594e1a62502a48".substring(0, Math.min("986d733f73d95810492027afa9594e1a62502a48".length(), 20));
        }
    ...
    public static String getGitRevisionString() {
        ...
        return "986d733f73d95810492027afa9594e1a62502a48 2026-01-19 12:14:48 (ZB)";
    }
    ...
    static {
        gameVersion = new GameVersion(42, 13, "");
