...
extends GameState {
        ...
        loader = new Thread(ThreadGroups.Workers, new Runnable(this){
            ...
            private void runInner() throws Exception {
                ...
                if (!GameServer.server) {
                    ...
                    if (newGame || IsoWorld.savedWorldVersion != 241) {
                        if (!newGame) {
    ...
    public void render() {
        ...
        if (GameClient.client) {
            ...
            TextManager.instance.DrawStringCentre(UIFont.Small, Core.getInstance().getScreenWidth() / 2, (float)(Core.getInstance().getScreenHeight() - 40) - font_height_small - 5.0f, str, 0.5, 0.5, 0.5, 1.0);
            if (GameClient.connection.getConnectionType() == UdpConnection.ConnectionType.Steam) {
                SpriteRenderer.instance.render(null, ((float)Core.getInstance().getScreenWidth() - this.width) / 2.0f, (float)(Core.getInstance().getScreenHeight() - 40) - font_height_small * 2.0f - 5.0f, this.width, 18.0f, 1.0f, 0.4f, 0.35f, 0.8f, null);
                TextManager.instance.DrawStringCentre(UIFont.Medium, Core.getInstance().getScreenWidth() / 2, (float)(Core.getInstance().getScreenHeight() - 40) - font_height_small * 2.0f - 5.0f, this.text, 0.1, 0.1, 0.1, 1.0);
            }
        } else if (!playerCreated && newGame && !Core.isLastStand()) {
            TextManager.instance.DrawStringCentre(UIFont.NewSmall, Core.getInstance().getScreenWidth() / 2, (float)(Core.getInstance().getScreenHeight() - 40) - font_height_small - 5.0f, Translator.getText("UI_Loading").replace(".", ""), 0.5, 0.5, 0.5, 1.0);
            TextManager.instance.DrawString(UIFont.NewSmall, Core.getInstance().getScreenWidth() / 2 + TextManager.instance.MeasureStringX(UIFont.Small, Translator.getText("UI_Loading").replace(".", "")) / 2 + 1, (float)(Core.getInstance().getScreenHeight() - 40) - font_height_small - 5.0f, this.loadingDot, 0.5, 0.5, 0.5, 1.0);
        }
        this.doQuickTips();
        if (this.stage == 0) {
        ...
        if (done && playerCreated && (!newGame || this.time >= 33.0f || Core.isLastStand() || "Tutorial".equals(Core.gameMode))) {
            ...
            if (GameWindow.activatedJoyPad == null || JoypadManager.instance.joypadList.isEmpty()) {
                TextManager.instance.DrawStringCentre(UIFont.NewLarge, Core.getInstance().getScreenWidth() / 2, (float)(baseline - 40) - font_height_small - 5.0f, Translator.getText("UI_ClickToSkip"), 1.0, 1.0, 1.0, this.clickToSkipAlpha);
            } else {
                ...
                TextManager.instance.DrawStringCentre(UIFont.Small, Core.getInstance().getScreenWidth() / 2, (float)(baseline - 40) - font_height_small - 5.0f, Translator.getText("UI_PressAToStart"), 1.0, 1.0, 1.0, this.clickToSkipAlpha);
            }
        }
        ActiveMods.renderUI();
        Core.getInstance().EndFrameUI();
